...
def people(number):
    return int(int(number)*adult+int(number)*kid*0.75)
...
with colc:
    st.header("""총합 : {}원
              """.format(f"{int(people(air_pre[0])+hot_pre[0]+car_pre[0]):,}"))
...
# 2023-08-20
    ...
    average_prices['price'] = average_prices['price'].apply(lambda x: '{:,}원'.format(int(x)))
...
with row4_1:
    ...
    average_prices_hotel['가격'] = average_prices_hotel['가격'].apply(lambda x: '{:,}원'.format(int(x)))

    # plotly 그래프 그리기
    hotel_type=average_prices_hotel['숙박유형'].tolist()
    colors = ['DarkOrange' if i == hot_category else 'Lavender' for i in hotel_type]
...
with row4_2:
    ...
    colors = ['DarkOrange' if i == hotel_score else 'Lavender' for i in hotel_type]
...
with row5_1:
    ...
    average_prices_car['가격'] = average_prices_car['가격'].apply(lambda x: '{:,}원'.format(int(x)))
